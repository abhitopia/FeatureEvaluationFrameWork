EXPERIMENT_NAME = 'VisitorFeature_log_median_1week'
...
def num_vds(visitor_info, which):
    if visitor_info == '':
        ...
        result = {
            'good': np.log(1 + float(len(weights[weights > 0]))),
            'bad': np.log(1 + float(len(weights[weights < 0])))
        }[which]
        return result
...
def sum_weights(visitor_info, which):
    if visitor_info == '':
        ...
        weights = Series(data=[float(ele.split('=')[1]) for ele in values])
        return {
            'good': np.log(1 + weights[weights > 0].sum()),
            'bad': np.log(1 + (-weights[weights < 0]).sum())
        }[which]
...
def time_proximity(visitor_info, which):
    if visitor_info == '':
        ...
        weights = Series(data=[float(ele.split('=')[1]) for ele in values])
        return {
             'good': np.log(1 + (1.0/(hours[weights > 0])).sum()),
             'bad': np.log(1 + (1.0/(hours[weights < 0])).sum())
        }[which]
...
def visit_score(visitor_info, which):
    if visitor_info == '':
        ...
        weights = Series(data=[float(ele.split('=')[1]) for ele in values])
        return {
            'good': np.log(1 + (weights[weights > 0]/(hours[weights > 0])).sum()),
            'bad': np.log(1 + (-weights[weights < 0]/(hours[weights < 0])).sum())
        }[which]
...
MODEL_CONFIG = {
    ...
    'visitor_features': {
        ...
        'aggregate values': {
            'mean_bad_count': lambda bad_count: bad_count.median(),
            'mean_good_count': lambda good_count: good_count.median(),
            'mean_good_weights': lambda sum_good_weights: sum_good_weights.median(),
            'mean_bad_weights': lambda sum_bad_weights: sum_bad_weights.median(),
            'mean_good_tp': lambda good_time_proximity: good_time_proximity.median(),
            'mean_bad_tp': lambda bad_time_proximity: bad_time_proximity.median(),
            'mean_good_vs': lambda good_visit_score: good_visit_score.median(),
            'mean_bad_vs': lambda bad_visit_score: bad_visit_score.median(),
            'mean_good_vsa': lambda good_visit_score_avg: good_visit_score_avg.median(),
            'mean_bad_vsa': lambda bad_visit_score_avg: bad_visit_score_avg.median()
        },
        'learners': {
